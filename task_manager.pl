...
% ========================================================
%  Implementation by Abhay


:- dynamic solutionOnCurrentNode/2, problemOnCurrentNode/1, agentPassedWithProblemID/1. % the agentPassedWithProblemID is not used.
...
% assertSolution may be for future use.
...
manager_handler( Name, Link, assertProblem( Type ) ):-
	...
	node_info( Nodename, _, _ ),
	( write( `Problem started at,` ), write( Nodename ), write( `,` ), write( Type )) ~> Ss,
	send_log( Ss ).
...
send_log1(Data):- connect_log(Sname),
			name(Data,Data_List),
			string_chars(Datum,Data_List),
			ssend(Sname,Datum),
			disconnect_log(Sname),
...
send_log1(Data):- write(`~M~JSend Log Failed with data`:Data),!.
